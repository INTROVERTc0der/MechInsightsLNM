...
export const getStudentData = createAsyncThunk("/auth/student/details", async (rollNo) => {
  try {
    const res = await axiosInstance.get(`/user_student/me`);
    return res?.data;
  } catch (error) {
    toast.error(error.message);
  }
...
export const forgetstudentPassword = createAsyncThunk(
  ...
  async (email) => {
    try {
      let res = axiosInstance.post("/user_student/reset", { email });
