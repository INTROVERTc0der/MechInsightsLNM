...
export const getfacultyData = createAsyncThunk("/auth/faculty/details", async () => {
  try {
    ...
    const res = await axiosInstance.get("/user_faculty/profile");
    console.log("hey");
    console.log("Profile info:", res.data.user);
    return res.data.user;
  } catch (error) {
    toast.error(error.message);
    throw error; // Ensure the error is propagated for proper error handling in Redux
  }
});
...
export const forgetfacultyPassword = createAsyncThunk(
  ...
  async (email) => {
    try {
      let res = axiosInstance.post("/faculty_user/forgetPassword", { email });
